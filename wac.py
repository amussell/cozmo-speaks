...
class WAC:
    ...
    def predictWord(self, features):
        wordPreds = [(word, self.wac[word].predict_proba(np.expand_dims(features, axis=0))[0][1]) for word in self.vocab()]
        mostProbWord = max(wordPreds, key=itemgetter(1))
        return mostProbWord
